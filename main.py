...
def jornada_cliente_lista(
    ...
    Retorna tabela paginada com abas: todos, pagantes, pendentes, expirados.
    """
    try:
        user_bots = db.query(BotModel).filter(BotModel.owner_id == current_user.id).all()
        ...
        lista.sort(key=lambda x: (
            0 if x["ultimo_status"] == "pagante" else 1 if x["ultimo_status"] == "pendente" else 2,
            -x["total_gasto"]
        ))
...
def jornada_cliente_mapa(
    ...
    Timeline: start → gerou PIX → pagou → order bump → upsell → downsell → remarketing
    """
    try:
        ...
        # 2-3. Gerou PIX + Pagou (Principal)
        if pedido_principal:
            timeline.append({
                "etapa": "gerou_pix", "titulo": "Gerou PIX",
                "descricao": f"Plano: {pedido_principal.plano_nome or 'N/A'} — R$ {float(pedido_principal.valor or 0):.2f}",
            ...
            timeline.append({
                ...
                "titulo": "Pagou (Principal)" if is_paid_p else "Não Pagou (Principal)",
                "descricao": f"Plano: {pedido_principal.plano_nome or 'N/A'} — R$ {float(pedido_principal.valor or 0):.2f}" if is_paid_p else "PIX expirou ou não foi pago",
            ...
            if ob_config and ob_config.ativo:
                ...
                timeline.append({
                    "etapa": "order_bump",
                    "titulo": "Order Bump" + (" — Aceito ✅" if tem_ob else " — Recusado ❌"),
                    "descricao": f"{ob_config.nome_produto or 'Produto extra'} — R$ {float(ob_config.preco or 0):.2f}" if tem_ob else "Não aceitou a oferta extra",
                    "data": clean_date(pedido_principal.data_aprovacao) if tem_ob else clean_date(pedido_principal.created_at),
                    "status": "completo" if tem_ob else "recusado",
                    "cor": "#10b981" if tem_ob else "#6b7280"
                })
        
        # 5. Upsell
        if up_config and up_config.ativo:
            if pedido_upsell:
                is_paid_up = pedido_upsell.status in ['approved', 'paid', 'active', 'expired']
                timeline.append({
                    "etapa": "upsell",
                    "titulo": "Upsell" + (" — Pagou ✅" if is_paid_up else " — Gerou PIX (Não Pagou)"),
                    "descricao": f"{up_config.nome_produto or 'Oferta Upsell'} — R$ {float(pedido_upsell.valor or 0):.2f}",
            ...
            elif pedido_principal and pedido_principal.status in ['approved', 'paid', 'active', 'expired']:
                timeline.append({
                    "etapa": "upsell", "titulo": "Upsell — Não Interagiu",
                    "descricao": f"Oferta de {up_config.nome_produto or 'Upsell'} foi enviada",
                    "data": None, "status": "nao_interagiu", "cor": "#6b7280"
                })
        
        # 6. Downsell
        if ds_config and ds_config.ativo:
            if pedido_downsell:
                is_paid_ds = pedido_downsell.status in ['approved', 'paid', 'active', 'expired']
                timeline.append({
                    "etapa": "downsell",
                    "titulo": "Downsell" + (" — Pagou ✅" if is_paid_ds else " — Gerou PIX (Não Pagou)"),
                    "descricao": f"{ds_config.nome_produto or 'Oferta Downsell'} — R$ {float(pedido_downsell.valor or 0):.2f}",
            ...
            elif pedido_upsell and pedido_upsell.status not in ['approved', 'paid', 'active', 'expired']:
                timeline.append({
                    "etapa": "downsell", "titulo": "Downsell — Não Interagiu",
                    "descricao": f"Oferta de {ds_config.nome_produto or 'Downsell'} foi enviada",
                    "data": None, "status": "nao_interagiu", "cor": "#6b7280"
                })
        ...
        total_pedidos = len([p for p in pedidos if p.status in ['approved', 'paid', 'active', 'expired']])
